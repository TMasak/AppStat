...
* $\Rightarrow$ $P(\mu_\star \in (g^{-1}(L_{\eta_\star}),g^{-1}({\eta_\star}))) = 1-\alpha$
...
* $\Rightarrow$ run $(g^{-1}(L_{\eta_\star}),g^{-1}({\eta_\star}))$ through the quantile function (of the response distribution estimated by the GLM) and report the minimum and maximum value as the prediction interval $(L_{Y_\star},U_{Y_\star})$
...
* prediction intervals are fairly useless for binary data (there CI for $\eta_\star$ is enough)
...
* Poisson (a.k.a. log-linear): how many goals will be scored is a given match
...
Several options for the response distribution:
